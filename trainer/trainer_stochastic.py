...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in enumerate(self.train_data_loader):
            ...
            text_embeds, video_features,video_embeds_pooled, text_embeds_stochastic, text_mean, text_log_var , target_loss = self.model(data, is_train=True , return_all_frames=True)
            ...
            if self.config.noloss_record:
                ...
                gen_log(model_path=self.config.model_path, log_name='log_ori_loss',
                        msg=loss.item())
                gen_log(model_path=self.config.model_path, log_name='log_dif_loss',
                        msg=target_loss.item())


            if batch_idx % self.log_step == 0:
                msg = ('Train Epoch: {} dl: {}/{} Total Loss: {:.6f}, Original Loss: {:.6f}, Diffusion Loss: {:.6f}'.format(
                    epoch,
                    batch_idx,
                    num_steps-1,
    ...
    def _valid_epoch_step(self, epoch, step, num_steps):
        ...
        with torch.no_grad():
            ...
            text_embeds_stochastic_allpairs = text_embeds_stochastic_allpairs.cpu()
            ...
            for (idx_vid, single_vid) in tqdm((enumerate(vid_embeds))):
                ...
                all_text_embed_stochastic, _, _ = self.model.stochastic(text_embeds, single_vid_repeat , all_row_text,'Test') # [bs_t, dim]
                all_text_embed_stochstic.append(all_text_embed_stochastic)
                all_text_embed_stochstic_arr = torch.stack(all_text_embed_stochstic, dim=0) # [#1, bs_t, dim]
                ...
                for i in range(all_text_embed_stochstic_arr.shape[1]):
                    selected_plane[i, :] = all_text_embed_stochstic_arr[0, i, :]
                selected_plane = selected_plane.cpu()

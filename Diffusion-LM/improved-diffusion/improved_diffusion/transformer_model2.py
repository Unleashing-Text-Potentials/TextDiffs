...
class TransformerNetModel2(nn.Module):
    ...
    def __init__(
        ...
        self.sims_linear = nn.Linear(12, config.hidden_size)

        self.sims_decoder = nn.Sequential(
    ...
    def forward(self, x, timesteps, y=None, src_ids=None, src_mask=None):
        ...
        # print( emb_inputs.size() )  #32*64*768
        emb_inputs = self.dropout(self.LayerNorm(emb_inputs))
        # print( emb_inputs.size() )  #32*64*768
        # print( self.conditional_gen ) False
        
        sims = y
        sims = self.sims_linear( sims )
        # print( emb_inputs.size() ) 32*64*768
        emb_inputs = torch.cat([emb_inputs , sims ], dim=-1)
        emb_inputs = self.sims_decoder( emb_inputs )

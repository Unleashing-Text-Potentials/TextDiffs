...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x,  sims_out, t, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        Apply the model to get p(x_{t-1} | x_t), as well as a prediction of
        the initial x, x_0.
        ...
        assert t.shape == (B,)
        # print(x.shape)
        model_output = model(x, self._scale_timesteps(t), y = sims_out , **model_kwargs)
    ...
    def training_losses_e2e(self, model, x_start, t, video_feature ,only_sims=None , model_kwargs=None, noise=None):
        """
        ...
        """
        #  model_path：/data/yangsijie/2024-10/10-teacher-diff-tmass/Diffusion-LM/improved-diffusion/improved_diffusion/transformer_model2.py
        assert 'input_ids' in model_kwargs
        input_ids = model_kwargs.pop('input_ids').to(t.device)
        # print( input_ids.size() ) 64*64
        x_start_mean = model.model.get_embeds(input_ids)
        # print( x_start_mean.size() )  64*64*16
        # print( self.model_arch ) transformer
